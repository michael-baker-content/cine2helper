// find-missing-low-threshold.mjs
// Discovers films not in curated lists using a low vote threshold (50),
// so you can assess quality of films that would be included at lower bars.
//
// Run: node scripts/find-missing-low-threshold.mjs
// Output is sorted by vote_count desc within each condition,
// with a rough quality tier label so you can scan quickly.
...
if (!TOKEN) { console.error('No TMDB_READ_ACCESS_TOKEN found in .env.local'); process.exit(1); }

// ── Existing curated IDs (copied from find-missing.mjs) ──────────────────────
...
// Vote count tiers — gives you a quick quality signal when scanning output
function tier(votes) {
  if (votes >= 1000) return '★★★';
  if (votes >= 200)  return '★★ ';
  if (votes >= 50)   return '★  ';
  return                     '   ';
}
...
async function discoverPage(genreId, dateGte, dateLte, page) {
...
for (const cond of CONDITIONS) {
  console.log(`\n${'═'.repeat(60)}`);
  console.log(`  ${cond.label} — films NOT in curated list (min 50 votes)`);
  ...
  while (page <= totalPages) {
    const data = await discoverPage(cond.genreId, cond.dateGte, cond.dateLte, page);
    totalPages = Math.min(data.total_pages ?? 1, 20); // cap at 20 pages (~400 results)
    ...
    for (const film of data.results ?? []) {
      if (!CURATED[cond.id].has(film.id)) {
        missing.push({
          ...
          rating: Number(film.vote_average).toFixed(1),
          popularity: Number(film.popularity).toFixed(0),
        });
      }
    }
  ...
  console.log(`\n  ${tier('9999')} = 1000+ votes  ${tier('500')} = 200–999  ${tier('100')} = 50–199\n`);
  console.log(`  ${'VOTES'.padStart(6)}  ${'RTG'.padStart(4)}  TIER  ID        TITLE (YEAR)`);
  console.log(`  ${'─'.repeat(70)}`);

  for (const f of missing) {
    const votes  = String(f.votes).padStart(6);
    const rating = String(f.rating).padStart(4);
    const t      = tier(f.votes);
    const id     = String(f.id).padEnd(9);
    console.log(`  ${votes}  ${rating}  ${t}  ${id} ${f.title} (${f.year})`);
  }

  console.log(`\n  Total missing: ${missing.length} films`);
}
